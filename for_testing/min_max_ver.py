...
class Regressor():
    ...
    def _preprocessor(self, x, y = None, training = False):
        ...
        if training:
            ...
            x_preprocessed = x-self.x_min
            x_preprocessed = x_preprocessed.div((self.x_max - self.x_min))
            x_preprocessed.ocean_proximity = n

            self.y_min = y.min(axis=0)
            self.y_max = y.max(axis=0)

            y_preprocessed = (y-self.y_min)
            y_preprocessed = (y_preprocessed).div((self.y_max - self.y_min))
    ...
    def predict(self, x):
        ...
        predict_value *= (self.y_max - self.y_min)
        predict_value += self.y_min
...
def RegressorHyperParameterSearch(x, y, no_of_epochs=10, optimizer_types=['Adam', 'AdaDelta'], neuron_lists=[15,8,4], activation_functions=['LeakyReLU', 'ReLU', 'eLU']) : 
...
def example_main():
    ...
    # regressor = Regressor(x_train, 0.01, "Adam", [50, 25, 20, 10, 5], 5000, "LeakyReLU")
    ...
    best_neur, best_learn_rate, best_optimiser, val_error, train_error, activ, model = RegressorHyperParameterSearch(x_train, y_train, no_of_epochs=500, optimizer_types=['Adam', 'AdaDelta'], neuron_lists=[50,30,25,25,10,5], activation_functions=['LeakyReLU', 'ReLU', 'ELU'])
    ...
    # print("Evaluating at the best model:" error)
